...
class DStar(Navigation):
    ...
    def reset(self):
        self._b = np.zeros(self._costmap)
        self._t = np.zeros(self._costmap)
        self._h = np.inf * np.ones(np.shape(self._costmap))
    ...
    def char(self):
        ...
        if self._costmap is not None or self._costmap is not np.array([]):
            s = s + "\nCostmap: " + np.shape(self._costmap) + ", Open list: " \
                + np.size(self._open_list)
        else:
            s = s + "\nCostmap: empty:"
